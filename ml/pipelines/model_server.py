...
REQUEST_COUNT = Counter('model_requests_total', 'Total model requests', ['model_name', 'version', 'endpoint'])
REQUEST_DURATION = Histogram('model_request_duration_seconds', 'Request duration', ['model_name', 'version'])
PREDICTION_COUNT = Counter('model_predictions_total', 'Total predictions made', ['model_name', 'version'])
ERROR_COUNT = Counter('model_errors_total', 'Total errors', ['model_name', 'version', 'error_type'])
MODEL_LOAD_TIME = Gauge('model_load_time_seconds', 'Time to load model', ['model_name', 'version'])
MEMORY_USAGE = Gauge('model_memory_usage_bytes', 'Memory usage', ['model_name', 'version'])
CPU_USAGE = Gauge('model_cpu_usage_percent', 'CPU usage percentage', ['model_name', 'version'])
    ...
    def get_model(self, model_name: str, version: str) -> Optional[BaseEstimator]:
        ...
        with self.lock:
            ...
            if key in self.cache:
                ...
                if load_time and datetime.now() - load_time < timedelta(hours=self.ttl_hours):
...
class ModelLoader:
    ...
    def load_model(self, model_name: str, version: str = "latest") -> BaseEstimator:
        ...
        try:
            ...
            MODEL_LOAD_TIME.labels(model_name=model_name, version=version).set(load_time)
    ...
    def _load_from_mlflow(self, model_name: str, version: str) -> Optional[BaseEstimator]:
    ...
    def get_prediction_stats(self, model_name: str, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "error_rate": len([p for p in recent_predictions if p["status"] == "error"]) / len(recent_predictions),
...
class ModelServer:
    ...
    def _setup_routes(self):
        ...
        def health_check():
            ...
            try:
                ...
                if memory_percent > 90 or cpu_percent > 90 or redis_status == "unhealthy":
        ...
        def predict():
            ...
            try:
                ...
                if metadata.get("explain", False):
                    explanation = self._explain_prediction(model, feature_array, features)
                ...
                error_response = PredictionResponse(
                    request_id=request_id,
                    model_name=data.get("model_name", "unknown") if 'data' in locals() else "unknown",
                    model_version=data.get("model_version", "unknown") if 'data' in locals() else "unknown",
                    prediction=None,
                    confidence=None,
                    explanation=None,
                    processing_time_ms=processing_time,
    ...
    def _prepare_features(self, features: Dict[str, Any], model: BaseEstimator) -> np.ndarray:
    ...
    def _explain_prediction(self, model: BaseEstimator, features: np.ndarray, 
        ...
        if hasattr(model, 'feature_importances_'):
            ...
            explanation["feature_importance"] = dict(zip(feature_names, importances.tolist()))
        ...
        if hasattr(model, 'coef_'):
            ...
            explanation["coefficients"] = dict(zip(feature_names, coefficients.tolist()))
            ...
            while True:
                try:
                    ...
                    for model_key in self.model_cache.cache.keys():
                        ...
                        MEMORY_USAGE.labels(model_name=model_name, version=version).set(memory_usage)
                        CPU_USAGE.labels(model_name=model_name, version=version).set(cpu_usage)
...
def create_server_config() -> Dict[str, Any]:
    ...
    return {
        ...
        } if os.getenv("DEFAULT_MODEL_NAME") else None
    }
