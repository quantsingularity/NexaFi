...
from src.models.user import PaymentMethod, Transaction, Wallet, WalletBalanceHistory, PaymentProcessor, RecurringPayment, ExchangeRate, db
...
def calculate_fees(amount, payment_method_type, processor_name='default'):
    ...
    fee_structures = {
        'card': {'percentage': Decimal('0.029'), 'fixed': Decimal('0.30')},  # 2.9% + $0.30
        'bank_account': {'percentage': Decimal('0.008'), 'fixed': Decimal('0.00')},  # 0.8%
        'digital_wallet': {'percentage': Decimal('0.025'), 'fixed': Decimal('0.00')},  # 2.5%
    }
...
def update_wallet_balance(wallet, amount, change_type, description, transaction_id=None):
...
def create_payment_method():
    ...
    try:
        ...
        if data['type'] not in valid_types:
            return jsonify({'error': f'Invalid payment method type. Must be one of: {valid_types}'}), 400
        ...
        if payment_method.is_default:
            PaymentMethod.query.filter_by(user_id=request.user_id, is_default=True)\
        ...
        db.session.rollback()
        return jsonify({'error': 'Failed to create payment method', 'details': str(e)}), 500
...
def get_payment_method(method_id):
    ...
    try:
        ...
        return jsonify({'error': 'Failed to get payment method', 'details': str(e)}), 500
        ...
        for field in updatable_fields:
            if field in data:
                if field == 'expires_at' and data[field]:
                    setattr(payment_method, field, datetime.strptime(data[field], '%Y-%m-%d'))
                else:
...
def get_wallet_history(currency):
    ...
    try:
        ...
        history = WalletBalanceHistory.query.filter_by(wallet_id=wallet.id)\
        ...
        return jsonify({'error': 'Failed to get wallet history', 'details': str(e)}), 500
...
def get_recurring_payments():
    ...
    try:
        recurring_payments = RecurringPayment.query.filter_by(user_id=request.user_id)\
                                                  .order_by(RecurringPayment.next_payment_date).all()
        
        return jsonify({
            'recurring_payments': [payment.to_dict() for payment in recurring_payments],
            'total': len(recurring_payments)
        }), 200
        
    except Exception as e:
        return jsonify({'error': 'Failed to get recurring payments', 'details': str(e)}), 500
...
def create_recurring_payment():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'Failed to create recurring payment', 'details': str(e)}), 500
...
def get_exchange_rates():
    ...
    try:
        ...
        # Get latest rates
        rates = query.filter(ExchangeRate.valid_from <= datetime.utcnow())\
                    .order_by(ExchangeRate.valid_from.desc()).all()
        ...
        return jsonify({
            'exchange_rates': [rate.to_dict() for rate in unique_rates.values()],
            'base_currency': base_currency,
            'timestamp': datetime.utcnow().isoformat()
        }), 200
        
    except Exception as e:
        return jsonify({'error': 'Failed to get exchange rates', 'details': str(e)}), 500
    ...
    try:
        ...
        return jsonify({
            'summary': {
                ...
                'average_transaction': float(total_volume / transaction_count) if transaction_count > 0 else 0
            },
            'by_type': by_type,
            'period': {
