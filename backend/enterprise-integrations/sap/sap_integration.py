...
class SAPAuthenticator:
    ...
    def _basic_authenticate(self) -> bool:
        ...
        test_url = f"{self.config.base_url}/sap/opu/odata/sap/API_BUSINESS_PARTNER/$metadata"
...
class SAPRFCConnector:
    ...
    def connect(self) -> bool:
        ...
        try:
            connection_params = {
                "ashost": self.config.base_url.replace("http://", "").replace("https://", ""),
    ...
    def read_table(self, table_name: str, fields: List[str] = None, 
                  where_clause: str = None, max_rows: int = 1000) -> List[Dict[str, Any]]:
...
class SAPODataConnector:
    ...
    def __init__(self, config: IntegrationConfig, sap_config: SAPConfig, authenticator: SAPAuthenticator):
        self.config = config
        self.sap_config = sap_config
    ...
    def get_entity_set(self, service_name: str, entity_set: str, 
                      filters: Dict[str, Any] = None, select: List[str] = None,
                      top: int = None, skip: int = None) -> List[Dict[str, Any]]:
        """Get data from OData entity set"""
        try:
            url = f"{self.config.base_url}/sap/opu/odata/sap/{service_name}/{entity_set}"
    ...
    def create_entity(self, service_name: str, entity_set: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def update_entity(self, service_name: str, entity_set: str, key: str, 
...
class SAPBusinessPartnerSync:
    ...
    def __init__(self, odata_connector: SAPODataConnector, data_transformer: DataTransformer):
    ...
    def sync_business_partners(self, filters: Dict[str, Any] = None) -> SyncResult:
        ...
        try:
            ...
            business_partners = self.odata_connector.get_entity_set(
                ...
                "A_BusinessPartner",
                filters=filters,
                select=["BusinessPartner", "BusinessPartnerCategory", "BusinessPartnerFullName",
            ...
            for bp in business_partners:
                try:
                    ...
                    self.logger.error(f"Failed to process business partner {bp.get('BusinessPartner')}: {str(e)}")
                    records_failed += 1
                    errors.append(str(e))
...
class SAPFinancialSync:
    ...
    def __init__(self, odata_connector: SAPODataConnector, rfc_connector: SAPRFCConnector,
    ...
    def sync_general_ledger(self, company_code: str, fiscal_year: str,
                           date_from: str = None, date_to: str = None) -> SyncResult:
        ...
        try:
            ...
            gl_data = self.rfc_connector.read_table(
                ...
                fields=["BUKRS", "BELNR", "GJAHR", "BUZEI", "HKONT", "DMBTR", "WRBTR", "BUDAT"],
...
class SAPIntegration(BaseIntegration):
    ...
    def __init__(self, config: IntegrationConfig, sap_config: SAPConfig, 
        ...
        if sap_config.enable_odata:
            self.odata_connector = SAPODataConnector(config, sap_config, self.authenticator)
        else:
            self.odata_connector = None
        
        # Initialize sync modules
        if self.odata_connector:
            self.bp_sync = SAPBusinessPartnerSync(self.odata_connector, self.data_transformer)
    ...
    def get_financial_documents(self, company_code: str, fiscal_year: str,
