...
class CreditScoreModel(db.Model):
    ...
    # Model configuration
    model_type = db.Column(db.String(50), default='xgboost')  # xgboost, random_forest, neural_network
    ...
    # Relationships
    scores = db.relationship('CreditScore', backref='model', cascade='all, delete-orphan')
...
class CreditScore(db.Model):
    ...
    model_id = db.Column(db.String(36), db.ForeignKey('credit_score_models.id'), nullable=False)
...
class LoanApplication(db.Model):
    ...
    # Application details
    loan_type = db.Column(db.String(50), nullable=False)  # personal, business, mortgage, auto
    ...
    # Relationships
    documents = db.relationship('LoanDocument', backref='application', cascade='all, delete-orphan')
    history = db.relationship('LoanApplicationHistory', backref='application', cascade='all, delete-orphan')
    ...
    def calculate_monthly_payment(self):
        ...
        if self.approved_amount and self.interest_rate and self.term_months:
            ...
            payment = self.approved_amount * (monthly_rate * (1 + monthly_rate) ** self.term_months) / \
...
class LoanPayment(db.Model):
    ...
    payment_status = db.Column(db.String(20), default='completed')  # pending, completed, failed, reversed
...
class RiskAssessment(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id = db.Column(db.String(36), nullable=False, index=True)
    assessment_type = db.Column(db.String(50), nullable=False)  # credit, operational, market, liquidity
    ...
    risk_level = db.Column(db.String(20), nullable=False)  # very_low, low, medium, high, very_high
    ...
    def get_mitigation_strategies(self):
        return json.loads(self.mitigation_strategies) if self.mitigation_strategies else []
...
class LoanDocument(db.Model):
    ...
    # Document details
    document_type = db.Column(db.String(50), nullable=False)  # income_statement, bank_statement, tax_return, etc.
