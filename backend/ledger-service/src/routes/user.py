...
from src.models.user import Account, JournalEntry, JournalEntryLine, FinancialPeriod, Budget, db
...
def create_default_chart_of_accounts(user_id):
    ...
    default_accounts = [
        # Assets
        {'code': '1000', 'name': 'Cash and Cash Equivalents', 'type': 'asset', 'subtype': 'current_asset', 'normal_balance': 'debit'},
        {'code': '1100', 'name': 'Accounts Receivable', 'type': 'asset', 'subtype': 'current_asset', 'normal_balance': 'debit'},
        {'code': '1200', 'name': 'Inventory', 'type': 'asset', 'subtype': 'current_asset', 'normal_balance': 'debit'},
        {'code': '1300', 'name': 'Prepaid Expenses', 'type': 'asset', 'subtype': 'current_asset', 'normal_balance': 'debit'},
        {'code': '1500', 'name': 'Property, Plant & Equipment', 'type': 'asset', 'subtype': 'fixed_asset', 'normal_balance': 'debit'},
        {'code': '1600', 'name': 'Accumulated Depreciation', 'type': 'asset', 'subtype': 'fixed_asset', 'normal_balance': 'credit'},
        
        # Liabilities
        {'code': '2000', 'name': 'Accounts Payable', 'type': 'liability', 'subtype': 'current_liability', 'normal_balance': 'credit'},
        {'code': '2100', 'name': 'Accrued Expenses', 'type': 'liability', 'subtype': 'current_liability', 'normal_balance': 'credit'},
        {'code': '2200', 'name': 'Short-term Debt', 'type': 'liability', 'subtype': 'current_liability', 'normal_balance': 'credit'},
        {'code': '2500', 'name': 'Long-term Debt', 'type': 'liability', 'subtype': 'long_term_liability', 'normal_balance': 'credit'},
        
        # Equity
        {'code': '3000', 'name': 'Owner\'s Equity', 'type': 'equity', 'subtype': 'capital', 'normal_balance': 'credit'},
        {'code': '3100', 'name': 'Retained Earnings', 'type': 'equity', 'subtype': 'retained_earnings', 'normal_balance': 'credit'},
        
        # Revenue
        {'code': '4000', 'name': 'Sales Revenue', 'type': 'revenue', 'subtype': 'operating_revenue', 'normal_balance': 'credit'},
        {'code': '4100', 'name': 'Service Revenue', 'type': 'revenue', 'subtype': 'operating_revenue', 'normal_balance': 'credit'},
        {'code': '4900', 'name': 'Other Income', 'type': 'revenue', 'subtype': 'non_operating_revenue', 'normal_balance': 'credit'},
        
        # Expenses
        {'code': '5000', 'name': 'Cost of Goods Sold', 'type': 'expense', 'subtype': 'cost_of_sales', 'normal_balance': 'debit'},
        {'code': '6000', 'name': 'Salaries and Wages', 'type': 'expense', 'subtype': 'operating_expense', 'normal_balance': 'debit'},
        {'code': '6100', 'name': 'Rent Expense', 'type': 'expense', 'subtype': 'operating_expense', 'normal_balance': 'debit'},
        {'code': '6200', 'name': 'Utilities Expense', 'type': 'expense', 'subtype': 'operating_expense', 'normal_balance': 'debit'},
        {'code': '6300', 'name': 'Marketing Expense', 'type': 'expense', 'subtype': 'operating_expense', 'normal_balance': 'debit'},
        {'code': '6400', 'name': 'Office Supplies', 'type': 'expense', 'subtype': 'operating_expense', 'normal_balance': 'debit'},
        {'code': '6500', 'name': 'Professional Services', 'type': 'expense', 'subtype': 'operating_expense', 'normal_balance': 'debit'},
        {'code': '6600', 'name': 'Insurance Expense', 'type': 'expense', 'subtype': 'operating_expense', 'normal_balance': 'debit'},
        {'code': '6700', 'name': 'Depreciation Expense', 'type': 'expense', 'subtype': 'operating_expense', 'normal_balance': 'debit'},
        {'code': '6800', 'name': 'Interest Expense', 'type': 'expense', 'subtype': 'non_operating_expense', 'normal_balance': 'debit'},
    ]
...
def update_account(account_id):
    ...
    try:
        ...
        # Update fields
        updatable_fields = ['account_name', 'account_type', 'account_subtype', 'description', 'is_active']
...
def get_journal_entries():
    ...
    try:
        ...
        return jsonify({
            'journal_entries': [entry.to_dict(include_lines=True) for entry in entries.items],
            'pagination': {
                'page': page,
                'per_page': per_page,
                'total': entries.total,
...
def create_journal_entry():
    ...
    try:
        ...
        entry = JournalEntry(
            ...
            entry_date=datetime.strptime(data.get('entry_date', datetime.now().strftime('%Y-%m-%d')), '%Y-%m-%d').date(),
        ...
        for i, line_data in enumerate(data['lines']):
            ...
            if debit_amount == 0 and credit_amount == 0:
                return jsonify({'error': f'Either debit or credit amount is required for line {i+1}'}), 400
            
            if debit_amount > 0 and credit_amount > 0:
                return jsonify({'error': f'Line {i+1} cannot have both debit and credit amounts'}), 400
...
def post_journal_entry(entry_id):
    ...
    try:
        entry = JournalEntry.query.filter_by(id=entry_id, user_id=request.user_id).first()
        ...
        if not entry.validate_entry():
            return jsonify({'error': 'Entry validation failed - debits must equal credits'}), 400
        ...
        db.session.rollback()
        return jsonify({'error': 'Failed to post journal entry', 'details': str(e)}), 500
...
def get_trial_balance():
    ...
    try:
        ...
        accounts = Account.query.filter_by(user_id=request.user_id, is_active=True)\
        ...
        return jsonify({'error': 'Failed to generate trial balance', 'details': str(e)}), 500
...
def get_balance_sheet():
    ...
    try:
        ...
        assets = Account.query.filter_by(user_id=request.user_id, account_type='asset', is_active=True)\
                             .order_by(Account.account_code).all()
        liabilities = Account.query.filter_by(user_id=request.user_id, account_type='liability', is_active=True)\
                                  .order_by(Account.account_code).all()
        equity = Account.query.filter_by(user_id=request.user_id, account_type='equity', is_active=True)\
                             .order_by(Account.account_code).all()
        ...
        return jsonify({'error': 'Failed to generate balance sheet', 'details': str(e)}), 500
...
def get_income_statement():
    ...
    try:
        ...
        revenue_accounts = Account.query.filter_by(user_id=request.user_id, account_type='revenue', is_active=True)\
                                       .order_by(Account.account_code).all()
        expense_accounts = Account.query.filter_by(user_id=request.user_id, account_type='expense', is_active=True)\
                                       .order_by(Account.account_code).all()
        ...
        for account in revenue_accounts:
            ...
            query = db.session.query(
                db.func.sum(JournalEntryLine.credit_amount - JournalEntryLine.debit_amount).label('balance')
        ...
        for account in expense_accounts:
            # Get balance for the period
            query = db.session.query(
                db.func.sum(JournalEntryLine.debit_amount - JournalEntryLine.credit_amount).label('balance')
            ).join(JournalEntry).filter(
                JournalEntryLine.account_id == account.id,
                JournalEntry.status == 'posted',
                JournalEntry.entry_date >= start_date,
        ...
        return jsonify({'error': 'Failed to generate income statement', 'details': str(e)}), 500
...
def initialize_accounts():
    ...
    try:
        ...
        return jsonify({'message': 'Default chart of accounts created successfully'}), 201
        
    except Exception as e:
        db.session.rollback()
        return jsonify({'error': 'Failed to initialize accounts', 'details': str(e)}), 500
