...
from validators.schemas import (
    validate_json_request, SanitizationMixin, Schema, fields, validate,
    AccountSchema, JournalEntrySchema, JournalEntryLineSchema, FinancialValidators
)
...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'nexafi-ledger-service-secret-key-2024')
...
class JournalEntry(BaseModel):
    ...
    def get_lines(self) -> List['JournalEntryLine']:
        ...
        return JournalEntryLine.find_all("journal_entry_id = ? ORDER BY line_number", (self.id,))
...
class ExchangeRate(BaseModel):
    ...
    @classmethod
    def get_rate(cls, from_currency: str, to_currency: str, rate_date: datetime = None) -> Decimal:
...
class EnhancedAccountSchema(AccountSchema):
    ...
    account_subtype = fields.Str(required=False, validate=validate.OneOf([
        'cash', 'bank', 'accounts_receivable', 'inventory', 'fixed_assets',
        'accounts_payable', 'accrued_liabilities', 'long_term_debt',
        'common_stock', 'retained_earnings', 'sales_revenue', 'service_revenue',
        'cost_of_goods_sold', 'operating_expenses', 'interest_expense'
    ]))
    currency = fields.Str(required=False, validate=FinancialValidators.validate_currency_code)
    opening_balance = fields.Raw(required=False, validate=FinancialValidators.validate_amount)

class EnhancedJournalEntrySchema(JournalEntrySchema):
    currency = fields.Str(required=False, validate=FinancialValidators.validate_currency_code)
    exchange_rate = fields.Decimal(required=False, validate=validate.Range(min=0.000001, max=999999))

class ExchangeRateSchema(SanitizationMixin, Schema):
    from_currency = fields.Str(required=True, validate=FinancialValidators.validate_currency_code)
    to_currency = fields.Str(required=True, validate=FinancialValidators.validate_currency_code)
    rate = fields.Decimal(required=True, validate=validate.Range(min=0.000001, max=999999))
...
class ReconciliationSchema(SanitizationMixin, Schema):
    ...
    statement_balance = fields.Raw(required=True, validate=FinancialValidators.validate_amount)
...
DEFAULT_CHART_OF_ACCOUNTS = {
    ...
    '1100': {'name': 'Checking Account', 'type': 'asset', 'subtype': 'bank', 'parent': '1000'},
    '1200': {'name': 'Savings Account', 'type': 'asset', 'subtype': 'bank', 'parent': '1000'},
    '1300': {'name': 'Accounts Receivable', 'type': 'asset', 'subtype': 'accounts_receivable'},
    ...
    '2100': {'name': 'Accounts Payable', 'type': 'liability', 'subtype': 'accounts_payable', 'parent': '2000'},
    '2200': {'name': 'Accrued Expenses', 'type': 'liability', 'subtype': 'accrued_liabilities', 'parent': '2000'},
    '2300': {'name': 'Long-term Debt', 'type': 'liability', 'subtype': 'long_term_debt'},
    
    # Equity
    '3000': {'name': 'Equity', 'type': 'equity'},
    '3100': {'name': 'Common Stock', 'type': 'equity', 'subtype': 'common_stock', 'parent': '3000'},
    '3200': {'name': 'Retained Earnings', 'type': 'equity', 'subtype': 'retained_earnings', 'parent': '3000'},
    
    # Revenue
    '4000': {'name': 'Revenue', 'type': 'revenue'},
    '4100': {'name': 'Sales Revenue', 'type': 'revenue', 'subtype': 'sales_revenue', 'parent': '4000'},
    '4200': {'name': 'Service Revenue', 'type': 'revenue', 'subtype': 'service_revenue', 'parent': '4000'},
    
    # Expenses
    '5000': {'name': 'Cost of Goods Sold', 'type': 'expense', 'subtype': 'cost_of_goods_sold'},
    '6000': {'name': 'Operating Expenses', 'type': 'expense', 'subtype': 'operating_expenses'},
    '6100': {'name': 'Salaries and Wages', 'type': 'expense', 'subtype': 'operating_expenses', 'parent': '6000'},
    '6200': {'name': 'Rent Expense', 'type': 'expense', 'subtype': 'operating_expenses', 'parent': '6000'},
    '6300': {'name': 'Utilities Expense', 'type': 'expense', 'subtype': 'operating_expenses', 'parent': '6000'},
    '7000': {'name': 'Interest Expense', 'type': 'expense', 'subtype': 'interest_expense'}
}
    ...
    for account_code, account_data in DEFAULT_CHART_OF_ACCOUNTS.items():
        ...
        if not existing:
            ...
            if 'parent' in account_data:
                parent_account = Account.find_one("account_code = ?", (account_data['parent'],))
                if parent_account:
...
@audit_action(AuditEventType.ACCOUNT_CREATE, "account_created", severity=AuditSeverity.MEDIUM)
def create_account():
...
def get_account_balance(account_id):
    ...
    return jsonify({
        ...
        'balance': str(balance),
        'currency': account.currency,
        'as_of_date': as_of_date.isoformat() if as_of_date else datetime.utcnow().isoformat()
    })
...
@audit_action(AuditEventType.JOURNAL_ENTRY_CREATE, "journal_entry_created", severity=AuditSeverity.HIGH)
def create_journal_entry():
    ...
    entry_number = f"JE-{datetime.utcnow().strftime('%Y%m%d')}-{uuid.uuid4().hex[:8].upper()}"
    ...
    for i, line_data in enumerate(lines_data):
        ...
        if debit_amount == 0 and credit_amount == 0:
            return jsonify({'error': f'Line {i+1}: Either debit or credit amount must be specified'}), 400
...
@require_auth
@require_permission('transaction:write')
@audit_action(AuditEventType.JOURNAL_ENTRY_POST, "journal_entry_posted", severity=AuditSeverity.HIGH)
...
@audit_action(AuditEventType.SYSTEM_CONFIG_CHANGE, "exchange_rate_updated", severity=AuditSeverity.MEDIUM)
def update_exchange_rate():
    ...
    existing_rates = ExchangeRate.find_all(
        ...
        (data['from_currency'], data['to_currency'], data.get('rate_date', datetime.utcnow().date()))
    )
...
@audit_action(AuditEventType.ACCOUNT_UPDATE, "reconciliation_created", severity=AuditSeverity.MEDIUM)
def create_reconciliation():
    ...
    book_balance = account.get_balance(datetime.combine(data['reconciliation_date'], datetime.min.time()))
...
def trial_balance():
    ...
    accounts = Account.find_all("is_active = 1 AND currency = ? ORDER BY account_code", (currency,))
...
def balance_sheet():
    ...
    assets = Account.find_all("account_type = 'asset' AND is_active = 1 AND currency = ? ORDER BY account_code", (currency,))
    liabilities = Account.find_all("account_type = 'liability' AND is_active = 1 AND currency = ? ORDER BY account_code", (currency,))
    equity = Account.find_all("account_type = 'equity' AND is_active = 1 AND currency = ? ORDER BY account_code", (currency,))
...
if __name__ == '__main__':
    ...
    os.makedirs('/home/ubuntu/nexafi_backend_refactored/ledger-service/data', exist_ok=True)
