...
class Account(db.Model):
    ...
    journal_lines = db.relationship(
        "JournalEntryLine", backref="account", cascade="all, delete-orphan"
    )
    ...
    def get_balance(self, as_of_date=None):
        ...
        query = (
            ...
            .join(JournalEntry)
            .filter(
                JournalEntryLine.account_id == self.id, JournalEntry.status == "posted"
            )
        )
...
class JournalEntry(db.Model):
    ...
    lines = db.relationship(
        "JournalEntryLine", backref="journal_entry", cascade="all, delete-orphan"
    )
...
class JournalEntryLine(db.Model):
    ...
    journal_entry_id = db.Column(
        db.String(36), db.ForeignKey("journal_entries.id"), nullable=False
    )
...
class Budget(db.Model):
    ...
    account_id = db.Column(db.String(36), db.ForeignKey("accounts.id"), nullable=False)
    period_id = db.Column(
        db.String(36), db.ForeignKey("financial_periods.id"), nullable=False
    )
