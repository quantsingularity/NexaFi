...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'nexafi-enhanced-auth-service-secret-key-2024')
...
class MFASetupSchema(SanitizationMixin, Schema):
    ...
    phone_number = fields.Str(required=False, validate=validate.Regexp(r'^\+[1-9]\d{1,14}$'))

class MFAVerifySchema(SanitizationMixin, Schema):
    token = fields.Str(required=True, validate=validate.Length(min=4, max=10))
    method = fields.Str(required=True, validate=validate.OneOf(['totp', 'sms', 'backup_code']))
...
class OAuth2AuthorizeSchema(SanitizationMixin, Schema):
    ...
    code_challenge_method = fields.Str(required=False, validate=validate.OneOf(['S256', 'plain']))

class OAuth2TokenSchema(SanitizationMixin, Schema):
    grant_type = fields.Str(required=True, validate=validate.OneOf(['authorization_code', 'refresh_token']))
...
def login():
    ...
    event = SecurityEvent(
        ...
        threat_level=ThreatLevel.HIGH if risk_score > 70 else ThreatLevel.MODERATE if risk_score > 40 else ThreatLevel.LOW
    )
    ...
    if data['username'] == 'demo@nexafi.com' and data['password'] == 'SecurePass123!':
        ...
        mfa_required = risk_score > 40 or security_level in [SecurityLevel.HIGH, SecurityLevel.CRITICAL]
...
@audit_action(AuditEventType.USER_LOGIN, "mfa_verification", severity=AuditSeverity.HIGH)
def verify_mfa():
...
def oauth2_token():
    ...
    if data['grant_type'] == 'authorization_code':
        ...
        # Find token record
        token_record = AccessToken.find_by_field('refresh_token_hash', hash_token(refresh_token))
...
def oauth2_userinfo():
    ...
    # Find token record
    token_record = AccessToken.find_by_field('access_token_hash', hash_token(access_token))
