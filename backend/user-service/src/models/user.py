...
class User(db.Model):
    ...
    audit_logs = db.relationship(
        "AuditLog", backref="user", cascade="all, delete-orphan"
    )
    custom_fields = db.relationship(
        "UserCustomField", backref="user", cascade="all, delete-orphan"
    )
...
class RolePermission(db.Model):
    ...
    permission_id = db.Column(
        db.String(36), db.ForeignKey("permissions.id"), nullable=False
    )
    granted_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class AuditLog(db.Model):
    ...
    )  # e.g., 'login', 'create_account', 'update_profile'
