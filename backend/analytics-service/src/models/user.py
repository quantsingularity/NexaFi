...
class Dashboard(db.Model):
    ...
    reports = db.relationship(
        "Report", backref="dashboard", cascade="all, delete-orphan"
    )
...
class Widget(db.Model):
    ...
    def is_cache_valid(self):
        if not self.last_cached or not self.cached_data:
            return False
        return (
            datetime.utcnow() - self.last_cached
...
class Report(db.Model):
    ...
    executions = db.relationship(
        "ReportExecution", backref="report", cascade="all, delete-orphan"
    )
...
class Metric(db.Model):
    ...
    history = db.relationship(
        "MetricHistory", backref="metric", cascade="all, delete-orphan"
    )
