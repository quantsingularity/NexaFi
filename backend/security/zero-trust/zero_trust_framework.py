...
class ContextAnalyzer:
    ...
    def __init__(self, redis_client: redis.Redis, db_session, geoip_db_path: str = None):
    ...
    def analyze_request_context(self, user_id: str, request_data: Dict[str, Any]) -> SecurityContext:
        ...
        try:
            ...
            trust_score = self._calculate_trust_score(user_id, device_id, ip_address, location)
            
            # Calculate risk score
            risk_score = self._calculate_risk_score(user_id, ip_address, user_agent, location)
    ...
    def _calculate_trust_score(self, user_id: str, device_id: str, 
                              ip_address: str, location: Dict[str, Any]) -> float:
    ...
    def _calculate_risk_score(self, user_id: str, ip_address: str, 
                             user_agent: str, location: Dict[str, Any]) -> float:
    ...
    def _get_device_trust(self, device_id: str, user_id: str) -> float:
        ...
        try:
            device = self.db_session.query(DeviceFingerprint).filter_by(
    ...
    def _get_ip_trust(self, ip_address: str) -> float:
        ...
        try:
            ...
            threat = self.db_session.query(ThreatIntelligence).filter_by(
                indicator_type='ip',
    ...
    def _get_behavior_trust(self, user_id: str) -> float:
        ...
        try:
            ...
            recent_behaviors = self.db_session.query(UserBehavior).filter(
            ...
            anomaly_scores = [b.anomaly_score for b in recent_behaviors if b.anomaly_score is not None]
    ...
    def _get_anomaly_risk(self, user_id: str, ip_address: str, location: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if request_count and int(request_count) > 100:  # More than 100 requests in window
    ...
    def _calculate_behavioral_score(self, user_id: str, request_data: Dict[str, Any]) -> float:
    ...
    def _get_compliance_status(self, user_id: str, device_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            device = self.db_session.query(DeviceFingerprint).filter_by(device_id=device_id).first()
...
class PolicyEngine:
    ...
    def evaluate_access(self, context: SecurityContext, resource: str, action: str) -> Tuple[AccessDecision, str]:
        ...
        try:
            ...
            for policy in sorted_policies:
                ...
                if self._evaluate_conditions(policy.conditions, context, resource, action):
                    self.logger.info(f"Policy matched: {policy.name} -> {policy.action.value}")
    ...
    def _evaluate_conditions(self, conditions: Dict[str, Any], context: SecurityContext, 
                           resource: str, action: str) -> bool:
        """Evaluate policy conditions against context"""
        try:
            for field, condition in conditions.items():
                if not self._evaluate_condition(field, condition, context, resource, action):
                    return False
            return True
    ...
    def _evaluate_condition(self, field: str, condition: Dict[str, Any], 
                          context: SecurityContext, resource: str, action: str) -> bool:
...
class ZeroTrustFramework:
    ...
    def evaluate_request(self, user_id: str, resource: str, action: str, 
                        request_data: Dict[str, Any]) -> Tuple[AccessDecision, SecurityContext, str]:
        """Evaluate request and return access decision"""
        try:
            # Analyze security context
            context = self.context_analyzer.analyze_request_context(user_id, request_data)
            
            # Evaluate access policy
            decision, policy_name = self.policy_engine.evaluate_access(context, resource, action)
            
            # Log security event
            self._log_security_event(context, resource, action, decision, policy_name)
            
            # Update user behavior tracking
            self._track_user_behavior(context, resource, action, decision == AccessDecision.ALLOW)
    ...
    def _log_security_event(self, context: SecurityContext, resource: str, 
                           action: str, decision: AccessDecision, policy_name: str):
        """Log security event"""
        try:
            event = SecurityEvent(
    ...
    def _track_user_behavior(self, context: SecurityContext, resource: str, 
        ...
        try:
            ...
            if device:
                ...
                device = DeviceFingerprint(
                    device_id=context.device_id,
                    user_id=context.user_id,
                    fingerprint_hash=hashlib.sha256(context.device_fingerprint.encode()).hexdigest(),
    ...
    def get_security_metrics(self) -> Dict[str, Any]:
        ...
        try:
            # Get recent events
            recent_events = self.db_session.query(SecurityEvent).filter(
            ...
            challenged_requests = len([e for e in recent_events if e.decision == "challenge"])
            
            # Calculate average scores
            trust_scores = [e.trust_score for e in recent_events if e.trust_score is not None]
            ...
            return {
                ...
                "deny_rate": denied_requests / total_requests if total_requests > 0 else 0,
                "challenge_rate": challenged_requests / total_requests if total_requests > 0 else 0,
...
def create_zero_trust_framework(database_url: str, redis_url: str, 
                               geoip_db_path: str = None) -> ZeroTrustFramework:
