...
from middleware.rate_limiter import rate_limit, add_rate_limit_headers
from middleware.auth import init_auth_manager, optional_auth
from logging.logger import setup_request_logging, get_logger, log_security_event
from audit.audit_logger import audit_logger, AuditEventType, AuditSeverity

app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'nexafi-api-gateway-secret-key-2024')
...
SERVICES = {
    ...
    'payment-service': {
        ...
        'health_endpoint': '/api/v1/health',
        'routes': ['/api/v1/payment-methods', '/api/v1/transactions', '/api/v1/wallets', 
                  '/api/v1/recurring-payments', '/api/v1/exchange-rates', '/api/v1/analytics'],
        'timeout': 30,
        'retry_count': 3,
        'circuit_breaker': {'failure_threshold': 5, 'recovery_timeout': 60}
    },
...
def forward_request(service_name, service_config, path, method, headers=None, data=None, params=None):
    ...
    for attempt in range(retry_count):
        try:
            ...
            logger.warning(f"Connection error calling {service_name}, attempt {attempt + 1}")
    ...
    for service_name, config in SERVICES.items():
        try:
            ...
            service_status[service_name] = {
                ...
                'circuit_breaker_state': circuit_breaker_state.get(service_name, {}).get('state', 'closed')
            }
        except Exception:
            service_status[service_name] = {
                'status': 'unavailable',
