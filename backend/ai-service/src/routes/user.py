...
from src.models.user import AIModel, AIPrediction, FinancialInsight, ConversationSession, ConversationMessage, FeatureStore, ModelTrainingJob, db
...
def calculate_credit_score(user_id, business_data):
    ...
    credit_score = int(base_score + revenue_factor + age_factor + employee_factor + payment_history_factor)
...
def predict_cash_flow():
    ...
    try:
        ...
        # Generate forecast
        forecast = generate_cash_flow_forecast(request.user_id, historical_data, days_ahead)
        
        # Get or create cash flow model
        model = AIModel.query.filter_by(name='cash_flow_forecast_v1', is_active=True).first()
        if not model:
            model = AIModel(
        ...
        prediction = AIPrediction(
            ...
            explanation={
                'key_factors': ['historical_trends', 'seasonal_patterns', 'business_growth'],
        ...
        return jsonify({
            ...
            'forecast': forecast,
            'summary': {
                'total_predicted_cash_flow': sum(f['predicted_cash_flow'] for f in forecast),
                'average_daily_cash_flow': sum(f['predicted_cash_flow'] for f in forecast) / len(forecast),
        ...
        db.session.rollback()
        return jsonify({'error': 'Failed to generate cash flow forecast', 'details': str(e)}), 500
...
def predict_credit_score():
    ...
    try:
        ...
        model = AIModel.query.filter_by(name='credit_scoring_v1', is_active=True).first()
        if not model:
            model = AIModel(
        ...
        db.session.rollback()
        return jsonify({'error': 'Failed to calculate credit score', 'details': str(e)}), 500
...
def get_financial_insights():
    ...
    try:
        ...
        query = FinancialInsight.query.filter_by(user_id=request.user_id, is_dismissed=False)
    ...
    try:
        ...
        if financial_data.get('cash_flow_trend') == 'declining':
            insights_to_create.append({
                ...
                'data_points': {
                    ...
                    'previous_cash_flow': financial_data.get('previous_cash_flow', 0),
        ...
        if financial_data.get('unusual_expenses'):
            insights_to_create.append({
                ...
                'data_points': {
                    ...
                    'average_amount': financial_data.get('avg_marketing_expenses', 0),
        ...
        insights_to_create.append({
            ...
            'data_points': {
                ...
                'estimated_additional_revenue': financial_data.get('potential_revenue', 5000)
            },
            'recommendations': [
...
def mark_insight_read(insight_id):
    ...
    try:
        insight = FinancialInsight.query.filter_by(id=insight_id, user_id=request.user_id).first()
        ...
        db.session.rollback()
        return jsonify({'error': 'Failed to mark insight as read', 'details': str(e)}), 500
...
def get_chat_sessions():
    ...
    try:
        sessions = ConversationSession.query.filter_by(user_id=request.user_id)\
                                           .order_by(ConversationSession.last_activity_at.desc()).all()
...
def create_chat_session():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'Failed to create chat session', 'details': str(e)}), 500
...
def get_chat_messages(session_id):
    ...
    try:
        session = ConversationSession.query.filter_by(id=session_id, user_id=request.user_id).first()
...
def generate_ai_response(user_message, context):
    ...
    elif any(word in user_message_lower for word in ['expense', 'expenses', 'spending']):
    ...
    return {
        ...
        'metadata': {
            ...
            'suggested_actions': ['view_dashboard', 'generate_report', 'schedule_review']
        },
...
def get_predictions():
    ...
    try:
        ...
        predictions = query.order_by(AIPrediction.created_at.desc())\
