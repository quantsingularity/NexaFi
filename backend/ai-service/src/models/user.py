...
class AIModel(db.Model):
    ...
    model_type = db.Column(db.String(50), nullable=False)  # cash_flow_forecast, credit_scoring, document_processing, etc.
    ...
    # Relationships
    predictions = db.relationship('AIPrediction', backref='model', cascade='all, delete-orphan')
...
class AIPrediction(db.Model):
    ...
    input_data = db.Column(db.JSON, nullable=False)  # Input features used for prediction
    ...
    execution_time_ms = db.Column(db.Integer)  # Prediction execution time in milliseconds
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class FinancialInsight(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id = db.Column(db.String(36), nullable=False, index=True)
    insight_type = db.Column(db.String(50), nullable=False)  # cash_flow_alert, expense_anomaly, revenue_trend, etc.
    title = db.Column(db.String(255), nullable=False)
    description = db.Column(db.Text, nullable=False)
    severity = db.Column(db.String(20), default='info')  # info, warning, critical
    category = db.Column(db.String(50))  # cash_flow, expenses, revenue, risk, opportunity
...
class FeatureStore(db.Model):
    ...
    feature_group = db.Column(db.String(50), nullable=False)  # financial, behavioral, demographic, etc.
    feature_value = db.Column(db.JSON, nullable=False)  # The actual feature value
    feature_type = db.Column(db.String(20), nullable=False)  # numeric, categorical, boolean, array
...
class ModelTrainingJob(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    model_id = db.Column(db.String(36), db.ForeignKey('ai_models.id'), nullable=False)
    job_type = db.Column(db.String(50), nullable=False)  # training, retraining, validation
