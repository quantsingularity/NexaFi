...
jest.mock("../../../frontend/web/src/lib/api", () => ({
  ...
  createJournalEntry: jest.fn(() =>
    Promise.resolve({ message: "Entry created" }),
  ),
}));
...
describe("AccountingModule", () => {
  ...
  test("allows adding a new account", async () => {
    ...
    apiClient.createAccount.mockResolvedValueOnce({
    ...
    fireEvent.change(screen.getByLabelText(/Description/i), {
      target: { value: "A new account for testing" },
    });
    ...
    await waitFor(() => {
      ...
      // Dialog should close; adapt this if your component uses a different behavior
  ...
  test("allows creating a new journal entry", async () => {
    ...
    apiClient.createJournalEntry.mockResolvedValueOnce({
    ...
    fireEvent.change(screen.getByLabelText(/Entry Date/i), {
    ...
    fireEvent.change(screen.getAllByLabelText(/Debit:/i)[0], {
      target: { value: "500" },
    });

    // Add second line item (Credit)
    fireEvent.click(screen.getByRole("button", { name: /Add Line Item/i }));
    fireEvent.change(screen.getAllByLabelText(/Account:/i)[1], {
      target: { value: "2" },
    }); // Revenue
    fireEvent.change(screen.getAllByLabelText(/Credit:/i)[1], {
      target: { value: "500" },
    });
    ...
    await waitFor(() => {
      ...
      // Dialog should close; adapt if your UI differs
      expect(
        screen.queryByText(/Create Journal Entry/i),
