...
class TestAccountModel:
    ...
    def test_account_balance_calculation(self):
        ...
        revenue_account = create_test_account(user_id, \'4000\', \'Revenue\', \'revenue\', \'credit\')
        ...
        line1 = JournalEntryLine(journal_entry_id=entry.id, account_id=cash_account.id, debit_amount=Decimal(\'100.00\'))
        line2 = JournalEntryLine(journal_entry_id=entry.id, account_id=revenue_account.id, credit_amount=Decimal(\'100.00\'))
...
class TestJournalEntryModel:
    ...
    def test_journal_entry_creation(self):
        ...
        line1 = JournalEntryLine(journal_entry_id=entry.id, account_id=cash_account.id, debit_amount=Decimal(\'200.00\'))
        line2 = JournalEntryLine(journal_entry_id=entry.id, account_id=revenue_account.id, credit_amount=Decimal(\'200.00\'))
        db.session.add_all([line1, line2])
        db.session.commit()
    ...
    def test_journal_entry_validation(self):
        ...
        db.session.flush()
        line_b1 = JournalEntryLine(journal_entry_id=entry_balanced.id, account_id=cash_account.id, debit_amount=Decimal(\'50.00\'))
        line_b2 = JournalEntryLine(journal_entry_id=entry_balanced.id, account_id=revenue_account.id, credit_amount=Decimal(\'50.00\'))
        ...
        db.session.flush()
        line_u1 = JournalEntryLine(journal_entry_id=entry_unbalanced.id, account_id=cash_account.id, debit_amount=Decimal(\'100.00\'))
        line_u2 = JournalEntryLine(journal_entry_id=entry_unbalanced.id, account_id=revenue_account.id, credit_amount=Decimal(\'50.00\'))
...
class TestBudgetModel:
    ...
    def test_budget_creation(self):
        user_id = \'user123\'
        account = create_test_account(user_id, \'6000\', \'Marketing Expense\', \'expense\', \'debit\')
        period = FinancialPeriod(
            user_id=user_id,
            period_name=\'FY2024\',
            start_date=date(2024, 1, 1),
