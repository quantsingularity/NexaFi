...
def generate_auth_token(user_id, expiration_delta=timedelta(hours=1)):
...
class TestAuthRoutes:
    ...
    def test_logout_success(self, client):
        ...
        with app.app_context():
            # In a real app, this should check for token blacklisting, but here we check session status
            session = UserSession.query.filter_by(session_token=token).first()
            assert session is not None  # The session should exist
            assert session.is_active == False  # And be marked inactive
...
class TestUserManagementRoutes:
    ...
    def test_unauthorized_access_admin_route(self, client):
    ...
    def test_forbidden_access_admin_route(self, client):
        """Test non-admin user accessing an admin-only route."""
        # Create a non-admin user (business_owner role)
        ...
        with app.app_context():
            ...
            # Assign the business_owner role, which lacks 'manage_users'
            user_role = UserRole(
        ...
        # Attempt to access the get all users route (requires admin permissions)
    ...
    def test_get_all_users_success(self, client):
        ...
        json_data = response.get_json()
        assert (
            len(json_data["users"]) >= 3
        )  # Admin + 2 new users + possibly the non-admin from a previous test.
    ...
    def test_delete_user_success(self, client):
        ...
        with app.app_context():
            ...
            # Assuming a soft delete implementation
...
class TestRolePermissionRoutes:
    ...
    def test_get_roles_success(self, client):
        # This route should be protected, checking with a standard user
