...
class TestAPIGatewayRoutes:
    ...
    def test_gateway_health_degraded(self, client, mock_requests):
        ...
        # Adjust the list length based on how many services are called in your health check
        ...
        response = client.get("/health")
        # Depending on logic, 503 is common for degraded/unhealthy
        assert response.status_code == 503
        json_data = response.get_json()
        assert json_data["status"] == "degraded"
        # Assuming ledger-service was the second call mocked above
    ...
    def test_proxy_user_service_get(self, client, mock_requests):
        ...
        # Fixed quote nesting inside f-string
