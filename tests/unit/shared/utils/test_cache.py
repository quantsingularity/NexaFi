...
# Assuming these classes/functions are correctly defined and imported
...
def cache_manager(mock_redis_client):
    ...
    # Note: Using patch.object for constants like this is a common, though sometimes debated, practice in testing.
    # It ensures the CacheManager is initialized with controlled settings.
    with patch.object(InfrastructureConfig, "REDIS_HOST", "localhost"), patch.object(
        InfrastructureConfig, "REDIS_PORT", 6379
    ), patch.object(InfrastructureConfig, "REDIS_DB", 0), patch.object(
        InfrastructureConfig, "REDIS_PASSWORD", None
    ), patch.object(
        InfrastructureConfig, "CACHE_DEFAULT_TIMEOUT", 300
    ), patch.object(
        InfrastructureConfig, "CACHE_KEY_PREFIX", "nexafi:"
    ):
        yield CacheManager()


class TestCacheManager:
    # All methods in this class are well-written and correct.

    def test_get_existing_key(self, cache_manager, mock_redis_client):
        mock_redis_client.get.return_value = json.dumps({"data": "test_value"}).encode(
            "utf-8"
        )
    ...
    def test_set_key(self, cache_manager, mock_redis_client):
        ...
        mock_redis_client.setex.assert_called_once_with(
            "nexafi:test_key",
            60,
            # Added default=str to json.dumps for robustness against non-serializable types (good practice)
    ...
    def test_set_key_default_timeout(self, cache_manager, mock_redis_client):
        ...
        mock_redis_client.setex.assert_called_once_with(
            "nexafi:test_key",
            300,  # Checks the default timeout from the fixture
            json.dumps({"data": "new_value"}, default=str).encode("utf-8"),
        )
    ...
    def test_clear_pattern(self, cache_manager, mock_redis_client):
        # Redis keys are returned as bytes
        ...
        # Checks that delete is called with the list of byte keys
...
class TestCachedDecorator:
    ...
    def test_cached_decorator_no_key_func(self, mock_cache, cache_manager):
        ...
        # --- REFINEMENT START ---
        # Recalculate the expected key using the known logic: join (func_name, args), then MD5 hash
        key_parts = ["test_func", "1", "2"]
        expected_cache_key = hashlib.md5(":".join(key_parts).encode()).hexdigest()
        # --- REFINEMENT END ---

        mock_cache.get.assert_called_once_with(expected_cache_key)
        # Asserts the return value (3) is set with the correct key and timeout (100)
    ...
    def test_cached_decorator_with_key_func(self, mock_cache, cache_manager):
        ...
        def custom_key_func(x, y):
            # The custom key function explicitly defines the key
        ...
        # Assert the custom key is used directly, without hashing or prefixing
    ...
    def test_cached_decorator_returns_cached_value(self, mock_cache, cache_manager):
        # Mocking the cache hit scenario
        mock_cache.get.return_value = "cached_result"

        @cached()
        def test_func_cached():
            # This line should NOT be executed if the cache hit is successful
            return "original_result"

        result = test_func_cached()
        assert result == "cached_result"
        mock_cache.get.assert_called_once()
        # Assert that the function wasn't executed and set was not called (cache hit)
        mock_cache.set.assert_not_called()


class TestCacheKeyForUser:
    # This method is extremely well-written as it explicitly handles and validates
    # the complex logic of argument ordering and hashing for the user-specific key function.
    ...
    def test_cache_key_for_user_generation(self):
        ...
        # 1. Manually construct the expected parts (user_id, args, sorted kwargs)
        sorted_kwargs = sorted(
            [(k, v) for k, v in {"kwarg2": kwarg2, "kwarg1": kwarg1}.items()]
        )
        expected_key_parts_sorted = [user_id, arg1]
        expected_key_parts_sorted.extend(f"{k}:{v}" for k, v in sorted_kwargs)

        # 2. Calculate the expected hash
        expected_cache_key_sorted = hashlib.md5(
            ":".join(expected_key_parts_sorted).encode()
        ).hexdigest()

        # 3. Call the function with non-sorted keyword arguments (kwarg2, kwarg1) to prove internal sorting works
        generated_key = cache_key_for_user(user_id, arg1, kwarg2=kwarg2, kwarg1=kwarg1)

        # 4. Assert the generated key matches the manually calculated, sorted key
